...
class Empty extends TweetSet:
  ...
  def mostRetweeted: Tweet = throw java.util.NoSuchElementException("mostRetweeted on empty set") 

  def descendingByRetweet = Nil

  def union(tw: TweetSet) = tw
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = 
    ...
    val accLeft = left.filterAcc(p, newAcc)
    right.filterAcc(p, accLeft)
  ...
  def mostRetweeted: Tweet = {
    ...
    val maxChild = if (leftMost.retweets > rightMost.retweets) leftMost else rightMost
  ...
  def union(tw: TweetSet): TweetSet =
    left.union(right.union(tw)).incl(elem)
...
object GoogleVsApple:
  ...
  lazy val googleTweets: TweetSet = {
    TweetReader.allTweets.filter((x: Tweet) => {
      google.exists(word => {
        val a = x.text.toLowerCase.contains(word.toLowerCase)
        x.text.toLowerCase.contains(word.toLowerCase)
      })
    })
  }
  lazy val appleTweets: TweetSet = {
    val a =TweetReader.allTweets
    val p = TweetReader.allTweets.filter((x: Tweet) => apple.exists(word => x.text.toLowerCase.contains(word.toLowerCase)))
    p
  }
