...
object Anagrams extends AnagramsInterface:
  ...
  def wordOccurrences(w: Word): Occurrences = 
    w.toLowerCase.groupBy(identity).map((c, s) => (c, s.length)).toList.sorted
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = 
    occurrences.foldRight(List[Occurrences](List()))((pair, acc) =>
      acc ++ (for
        occ <- acc
        i <- 1 to pair._2
      yield (pair._1, i) :: occ)
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = 
    val yMap = y.toMap
    x.map((c, count) => (c, count - yMap.getOrElse(c, 0)))
     .filter(_._2 > 0)
    ...
    def anagrams(occurrences: Occurrences): List[Sentence] =
      if occurrences.isEmpty then List(List())
        ...
        for
          ...
          rest <- anagrams(subtract(occurrences, comb))
        yield word :: rest
